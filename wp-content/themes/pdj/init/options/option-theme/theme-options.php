...
class PDJSettingsPage {
  ...
  */
  public function pdj_page_init() {
    ...
    // Facebook Setting
    add_settings_section(
      'pdj_theme_setting', // ID
      __('Theme Setting', 'pdj_theme'), // Title
      array( $this, 'pdj_theme_print_section_info' ), // Callback
      'pdj-setting-admin' // Page
    );

    add_settings_field(
      'pdj_tour_canellation_policy',
      __('Tour Canellation Policy', 'pdj_theme'),
      array( $this, 'pdj_form_textarea' ), // Callback
      'pdj-setting-admin', // Page
      'pdj_theme_setting',
      'pdj_tour_canellation_policy'
    );
  ...
  public function pdj_sanitize( $input ) {
    ...
    if( isset( $input['pdj_tour_canellation_policy'] ) )
      $new_input['pdj_tour_canellation_policy'] = $input['pdj_tour_canellation_policy'];

    if( isset( $input['pdj_tour_attention'] ) )
      $new_input['pdj_tour_attention'] = $input['pdj_tour_attention'] ;
  ...
  */
  ...
  public function pdj_theme_print_section_info() {
    echo __("", 'pdj_theme');
  }
  ...
  */
  ...
  public function pdj_form_textarea($name) {
    $value = isset($this->options[$name]) ? esc_attr($this->options[$name]) : '';
    printf('<textarea cols="90" rows="4" type="textarea" id="form-id-%s" name="pdj_board_settings[%s]">%s</textarea>', $name, $name, $value);
  }

  /*public function pdj_form_editor_mce($name) {
    $value = isset($this->options[$name]) ? esc_attr($this->options[$name]) : '';
    printf('<textarea cols="90" rows="4" type="textarea" id="form-id-%s" name="pdj_board_settings[%s]">%s</textarea>', $name, $name, $value);
    //echo wp_editor( $value, $name, array('textarea_name' => $name)  );
  }*/
}
