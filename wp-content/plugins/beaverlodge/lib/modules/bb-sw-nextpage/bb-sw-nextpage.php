...
define( 'SW_NEXTPAGE_MODULE_DIR', plugin_dir_path( __FILE__ ) );
define( 'SW_NEXTPAGE_MODULE_URL', plugins_url( '/', __FILE__ ) );

function sw_nextpage_module() {
    if ( class_exists( 'FLBuilder' ) ) {
        require_once 'includes/bb-sw-nextpage-module.php';
    }
}
add_action( 'init', 'sw_nextpage_module' );
...
function next_page_add_pages() {
    // Add a new submenu under Beaverlodge
	$css = add_submenu_page('beaverlodge_modules', 'Next Page', 'Next Page', 'manage_options', 'next-page', 'next_page_options');
...
function load_fontawesome_style() {
        ...
        wp_register_style( 'foundation', '//cdnjs.cloudflare.com/ajax/libs/foundicons/3.0.0/foundation-icons.min.css' );
